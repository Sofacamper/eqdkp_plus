 ...
 * -----------------------------------------------------------------------
 * Began:		2002
 * Date:		$Date: 2012-12-17 12:22:26 +0100 (Mon, 17 Dec 2012) $
 * -----------------------------------------------------------------------
 ...
 * @version		$Rev: 12601 $
 *
 * $Id: manage_users.php 12601 2012-12-17 11:22:26Z godmod $
 */
...
class Manage_Users extends page_generic {
	...
	public function submit() {
		...
		if($user_id) {
			$this->pdh->put('user', 'update_user_settings', array($user_id));
		} else {
			$password = ($this->in->get('new_user_password1') == "") ? random_string() : $this->in->get('new_user_password1');
			...
			$customArray['hide_shop']		= $this->in->get('hide_shop', 0);
			$customArray['hide_mini_games']	= $this->in->get('hide_mini_games', 0);
			...
			$arrData = array(
				...
				'user_alimit'			=> $this->in->get('user_alimit'),
				'user_elimit'			=> $this->in->get('user_elimit'),
				'user_ilimit'			=> $this->in->get('user_ilimit'),
				'user_nlimit'			=> $this->in->get('user_nlimit'),
				'user_rlimit'			=> $this->in->get('user_rlimit'),
				'user_style'			=> $this->in->get('user_style'),
				'user_timezone'			=> $this->in->get('user_timezone'),
				'user_lang'				=> $this->in->get('user_lang'),
				'user_active'			=> 1,
				'first_name'			=> $this->in->get('first_name'),
				'last_name'				=> $this->in->get('last_name'),
				'country'				=> $this->in->get('country'),
				'town'					=> $this->in->get('town'),
				'state'					=> $this->in->get('state'),
				'ZIP_code'				=> $this->in->get('ZIP_code', 0),
				'phone'					=> $this->in->get('phone'),
				'cellphone'				=> $this->in->get('cellphone'),
				'address'				=> $this->in->get('address'),
				'allvatar_nick'			=> $this->in->get('allvatar_nick'),
				'icq'					=> $this->in->get('icq'),
				'skype'					=> $this->in->get('skype'),
				'msn'					=> $this->in->get('msn'),
				'irq'					=> $this->in->get('irq'),
				'gender'				=> $this->in->get('gender', 0),
				'birthday'				=> $this->in->get('birthday'),
				'user_date_time'		=> $this->in->get('user_date_time', $this->user->lang('style_time')),
				'user_date_short'		=> $this->in->get('user_date_short', $this->user->lang('style_date_short')),
				'user_date_long'		=> $this->in->get('user_date_long', $this->user->lang('style_date_long')),
				'privacy_settings'		=> serialize($privArray),
				'custom_fields'			=> serialize($customArray),
				'plugin_settings'		=> serialize($plugin_settings),
			);

				$user_id = $this->pdh->put('user', 'insert_user', array($arrData, true, false));
		}
		...
		if ($password OR ($this->config->get('account_activation') == 2 ) && ( $this->pdh->get('user', 'active', array($user_id)) < $this->in->get('user_active'))){
			...
			if($this->email->SendMailFromAdmin($this->in->get('user_email'), $this->user->lang('email_subject_activation_none'), 'register_activation_none.html', $bodyvars)) {
			...
			foreach($fieldsetdata as $fieldsetname=>$fielddata){
				...
				foreach($fielddata as $name=>$confvars){
					...
					$this->tpl->assign_block_vars('tabs.fieldset.field', array(
						...
						'FIELD'		=> $this->html->generateField($confvars, $name, ((isset($confvars['no_value'])) ? '' : (isset($this->user_data[$name]) ? $this->user_data[$name] : '')), $no_lang),
