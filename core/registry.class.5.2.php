 ...
 * -----------------------------------------------------------------------
 * Began:		2011
 * Date:		$Date: 2012-12-07 16:40:08 +0100 (Fri, 07 Dec 2012) $
 * -----------------------------------------------------------------------
 ...
 * @version		$Rev: 12567 $
 * 
 * $Id: registry.class.5.2.php 12567 2012-12-07 15:40:08Z hoofy_leon $
 */
...
final class registry extends super_registry{
	...
	public static function destruct($class, $class_hash='') {
		...
		foreach(array_keys(self::$loaded) as $classname) {
			$deps = registry::get_const('dep_'.$classname);
			if(!empty($deps)) {
	...
	private static function _destruct($class, $class_hash='') {
		...
		if($class_hash) unset(self::$inst[$class][$class_hash]);
